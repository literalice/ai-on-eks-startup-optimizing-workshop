...
**Goal / 目的:** determine how the startup time divides into stages, so that the later
steps have figures to be compared against.

起動時間が段階ごとにどう分かれるかを確認し、以降のステップの比較対象となる数字を得ます。
...
- The throughput figure was 98 MB/s. The instance supports up to 25 Gbps, so the pull
  was not limited by network bandwidth. Layers are downloaded and unpacked one at a
  time, which steps 2 and 3 address in different ways.
