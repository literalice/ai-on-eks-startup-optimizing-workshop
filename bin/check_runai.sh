...
#   bin/check_runai.sh
#
# Run this after a soci run, so the node already has the image and the check takes
# a few seconds rather than a cold multi-gigabyte pull. Pinned to the soci nodepool
# for that reason; if no soci node exists it will provision one and take
